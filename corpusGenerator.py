...
def get_articles_by_tag(api_key, tag_id, target_articles, page_size=200):
    ...
    while total_fetched < target_articles:
        ...
        if response.status_code == 200:
            ...
            print(
                f"Fetched page {page} for tag {tag_id} with {len(articles)} articles."
            )
        else:
...
def get_articles_from_multiple_tags(api_key, tags):
    all_articles = {}
    for tag in tags:
        random_article_count = random.randint(
            900, 1000
        )  # Random number of articles, no real reason
        print(f"Fetching {random_article_count} articles for tag: {tag}")
        articles = get_articles_by_tag(
            api_key, tag, target_articles=random_article_count
        )
...
def visualize_avg_length_per_tag(articles_by_tag):
    ...
    for tag, articles in articles_by_tag.items():
        lengths = [
            len(article["body"].split()) for article in articles if article["body"]
        ]
